...
const defaultUsers = [
  ...
  },
  {
    ...
    active: true,
    verified: true,
    institutionConfig: { alertThreshold: 40 },
  },
];
...
function saveUsersToStorage() {
  ...
  let data = stored
    ? JSON.parse(stored)
    : { emotionalRecords: [], favoriteResources: {}, auditLog: [] };
...
function login(email, password, remember) {
  ...
  console.log(
    ...
    appData.users.map((u) => `${u.email} (${u.role})`),
  );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("googleLoginBtn")?.addEventListener("click", () => {
    showMessage(
      "Inicio con Google (demo) - Usa docente@demo.com / 123456",
      false,
    );
  });
  document.getElementById("facebookLoginBtn")?.addEventListener("click", () => {
    showMessage(
      "Inicio con Facebook (demo) - Usa directivo@demo.com / 123456",
      false,
    );
  });
});
